...
export default {
  ...
  data () {
    return {
      ...
      personSelectList: [],
      personId: undefined,
      tempPersonId: undefined,
      tempPersonDataSource: []
    }
  },
  props: {
    registrant: {
  ...
  methods: {
        ...
        getPersonSelectList(params).then(res => {
          if (res.code === 200) {
            if (res.body && res.body.length > 0) {
              ...
              this.triggerChange(personId, record)
            }
          }
        })
      } else { // 工号姓名都不传
          ...
          if (res.code === 200) {
            if (res.body != null || res.body !== undefined) {
              ...
              if (res.body.length === 1) {
                this.$refs.selector.$listeners.select(res.body[0].personId)
                this.tempPersonId = res.body[0].personId
                this.tempPersonDataSource = res.body
              } else {
                this.tempPersonId = undefined
                this.tempPersonDataSource = []
              }
            }
          } else {
    ...
    handleBlur () {
      if (this.tempPersonId) {
        this.personId = this.tempPersonId
        this.personSelectList = [...this.tempPersonDataSource]
        const record = this.tempPersonDataSource.find(item => item.personId === this.tempPersonId)
        this.$emit('setPerson', record)
        this.triggerChange(this.tempPersonId, record)
        this.tempPersonId = undefined
        this.tempPersonDataSource = []
      }
    },
