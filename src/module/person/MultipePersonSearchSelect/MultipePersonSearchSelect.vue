...
<script>
import { findPersonSelectList, findPersonsWithPersonIds } from '@/api/common'
...
export default {
  ...
  mounted () {
    if (this.record[this.customKey]) {
      ...
      if (this.initLoading) {
        this.loadPersonWithPersonIds(this.userIds)
      } else {
  ...
  methods: {
    ...
    loadPersonWithPersonIds (personIds) {
      findPersonsWithPersonIds({ personIds: personIds }).then((res) => {
        if (res.code === 200) {
          if (res.body && res.body.length > 0) {
            this.userIds.forEach(userId => {
              var target = res.body.find(person => person.personId === userId)
              if (target) {
                this.userNames.push(target.personName)
              }
            })
            const deptNames = this.record.userDeptName ? this.record.userDeptName.split(',') : []
            for (var i in this.userIds) {
