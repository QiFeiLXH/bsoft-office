<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 6140, y: listScrollHeight }">
      >
...
import { getContractLevelworkloadAllByDeppByYear, exportContractLevelworkloadAllByDeppByYear } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getContractLevelworkloadAllByDeppByYear(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-B11' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelworkloadAllByDeppByYear(params).then(res => {
