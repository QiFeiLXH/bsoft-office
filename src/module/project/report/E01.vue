<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2700, y: listScrollHeight }"
...
<script>
import { getProjectLevelBonusAllSubtotal, exportProjectLevelBonusAllSubtotal } from '@/api/projectReport_bonus'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      const params = {}
      getProjectLevelBonusAllSubtotal(Object.assign(params, this.queryParam))
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E01' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelBonusAllSubtotal(params).then(res => {
