<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 6910, y: listScrollHeight }">
      >
...
import { getProjectLevelworkloadAllByMonth, exportProjectLevelworkloadAllByMonth } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getProjectLevelworkloadAllByMonth(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-B03' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelworkloadAllByMonth(params).then(res => {
