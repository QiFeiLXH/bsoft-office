<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2160, y: listScrollHeight }"
...
<script>
import { getContractLevelBonusDepSubtotal, exportContractLevelBonusDepSubtotal } from '@/api/projectReport_bonus'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      getContractLevelBonusDepSubtotal(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E10' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelBonusDepSubtotal(params).then(res => {
