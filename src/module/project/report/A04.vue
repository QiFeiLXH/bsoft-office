<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 3190, y: listScrollHeight }"
...
<script>
import { doLoadProjectAllDepSubTotal, exportProjectAllDepSubTotal } from '@/api/projectReport_all'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      doLoadProjectAllDepSubTotal(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-A04' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectAllDepSubTotal(Object.assign(params, this.queryParam)).then(res => {
