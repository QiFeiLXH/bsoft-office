<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2240, y: listScrollHeight }"
...
<script>
import { getContractLevelBonusAllMonth, exportContractLevelBonusAllMonth } from '@/api/projectReport_bonus'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      getContractLevelBonusAllMonth(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E09' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelBonusAllMonth(params).then(res => {
