<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2650, y: listScrollHeight }"
...
<script>
import { doLoadContractAllDepMonth, exportContractAllDepMonth } from '@/api/projectReport_all'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      doLoadContractAllDepMonth(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-A12' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractAllDepMonth(Object.assign(params, this.queryParam)).then(res => {
