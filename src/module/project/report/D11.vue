...
<script>
import { findContractLevelReimbursementCostDepYear, exportContractLevelReimbursementCostDepYear, getCostSubjectCloumns } from '@/api/projectReport_reimbursement'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      const params = {}
      findContractLevelReimbursementCostDepYear(Object.assign(params, this.queryParam))
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-D11' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelReimbursementCostDepYear(params).then(res => {
