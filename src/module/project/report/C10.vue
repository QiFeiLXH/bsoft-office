...
import { getCmoneyAllByDep, exportCmoneyAllByDep } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getCmoneyAllByDep(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-C10' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportCmoneyAllByDep(params).then(res => {
