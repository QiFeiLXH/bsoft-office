<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2340, y: listScrollHeight }">
      >
...
import { getCmoneyAllByDepByMonth, exportCmoneyAllByDepByMonth } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getCmoneyAllByDepByMonth(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-C12' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportCmoneyAllByDepByMonth(params).then(res => {
