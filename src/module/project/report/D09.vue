<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 9450, y: listScrollHeight }"
...
<script>
import { findContractLevelReimbursementCostAllMonth, exportContractLevelReimbursementCostAllMonth, getCostSubjectCloumns } from '@/api/projectReport_reimbursement'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      const params = {}
      findContractLevelReimbursementCostAllMonth(Object.assign(params, this.queryParam))
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-D09'// 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelReimbursementCostAllMonth(params).then(res => {
