<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 6980, y: listScrollHeight }">
      >
...
import { getProjectLevelworkloadAllByDepByMonth, exportProjectLevelworkloadAllByDepByMonth } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getProjectLevelworkloadAllByDepByMonth(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-B06' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelworkloadAllByDepByMonth(params).then(res => {
