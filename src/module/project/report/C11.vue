...
import { getCmoneyAllByDepByYear, exportCmoneyAllByDepByYear } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getCmoneyAllByDepByYear(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-C11' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportCmoneyAllByDepByYear(params).then(res => {
