<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 6070, y: listScrollHeight }">
      >
...
import { getContractLevelworkloadAllByYear, exportContractLevelworkloadAllByYear } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getContractLevelworkloadAllByYear(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-B08' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelworkloadAllByYear(params).then(res => {
