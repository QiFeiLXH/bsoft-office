<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 3080, y: listScrollHeight }"
...
<script>
import { doLoadProjectAllSubTotal, exportProjectAllSubTotal } from '@/api/projectReport_all'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      const params = {}
      doLoadProjectAllSubTotal(Object.assign(params, this.queryParam))
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-A01'// 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectAllSubTotal(params).then(res => {
