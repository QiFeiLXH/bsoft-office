<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 6000, y: listScrollHeight }">
      >
...
import { getContractLevelworkloadAll, exportContractLevelworkloadAll } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getContractLevelworkloadAll(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-B07' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelworkloadAll(params).then(res => {
