<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 3100, y: listScrollHeight }"
...
<script>
import { getProjectLevelBonusDepMonth, exportProjectLevelBonusDepMonth } from '@/api/projectReport_bonus'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      getProjectLevelBonusDepMonth(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E06' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelBonusDepMonth(params).then(res => {
