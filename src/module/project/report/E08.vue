...
<script>
import { getContractLevelBonusAllYear, exportContractLevelBonusAllYear } from '@/api/projectReport_bonus'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      getContractLevelBonusAllYear(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E08' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelBonusAllYear(params).then(res => {
