<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2980, y: listScrollHeight }">
      >
...
import { getPmoneyAllByMonth, exportPmoneyAllByMonth } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getWidthAndHeight () {
      ...
      this.listScrollHeight = window.innerHeight - 322
    },
    getList () {
      this.isLoading = true
      ...
      getPmoneyAllByMonth(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-C03' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportPmoneyAllByMonth(params).then(res => {
