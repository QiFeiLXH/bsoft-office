<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2260, y: listScrollHeight }"
...
<script>
import { getContractLevelBonusDepYear, exportContractLevelBonusDepYear } from '@/api/projectReport_bonus'
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        {
          key: 'projectBonusYear',
          width: 100,
          dataIndex: 'projectBonusYear',
          title: '项目奖金年度',
          align: 'center'
        },
        {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      getContractLevelBonusDepYear(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E11' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelBonusDepYear(params).then(res => {
