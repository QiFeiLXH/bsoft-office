<template>
  <a-card
    ...
    <a-table
      ...
      <template v-for="costSubject in costSubjects" :slot="costSubject.costSubject" slot-scope="text">
        {{ text | NumberFormat }}
      </template>
    </a-table>
    <footer-tool-bar>
...
<script>
import { findProjectLevelReimbursementCostAllSubtotal, exportProjectLevelReimbursementCostAllSubtotal, getCostSubjectCloumns } from '@/api/projectReport_reimbursement'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      const params = {}
      findProjectLevelReimbursementCostAllSubtotal(Object.assign(params, this.queryParam))
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-D01' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelReimbursementCostAllSubtotal(params).then(res => {
    ...
    initColumns () {
      ...
      this.costSubjects.forEach(item => {
        dynaicColumn = {
          ...
          // scopedSlots: { customRender: item.costSubject }
        }
