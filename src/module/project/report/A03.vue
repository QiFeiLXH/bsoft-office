<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 3240, y: listScrollHeight }"
...
<script>
import { doLoadProjectAllSubTotalMonth, exportProjectAllSubTotalMonth } from '@/api/projectReport_all'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      doLoadProjectAllSubTotalMonth(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-A03' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectAllSubTotalMonth(Object.assign(params, this.queryParam)).then(res => {
