<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2140, y: listScrollHeight }">
      >
...
import { getCmoneyAllByYear, exportCmoneyAllByYear } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getCmoneyAllByYear(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-C08' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportCmoneyAllByYear(params).then(res => {
