<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2940, y: listScrollHeight }"
...
<script>
import { getProjectLevelBonusAllMonth, exportProjectLevelBonusAllMonth } from '@/api/projectReport_bonus'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      getProjectLevelBonusAllMonth(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E03' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelBonusAllMonth(params).then(res => {
