<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 3270, y: listScrollHeight }"
...
<script>
import { doLoadProjectAllDepYear, exportProjectAllDepYear } from '@/api/projectReport_all'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      doLoadProjectAllDepYear(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-A05' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectAllDepYear(Object.assign(params, this.queryParam)).then(res => {
