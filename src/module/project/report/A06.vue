<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 3350, y: listScrollHeight }"
...
<script>
import { doLoadProjectAllDepMonth, exportProjectAllDepMonth } from '@/api/projectReport_all'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      doLoadProjectAllDepMonth(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-A06' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectAllDepMonth(Object.assign(params, this.queryParam)).then(res => {
