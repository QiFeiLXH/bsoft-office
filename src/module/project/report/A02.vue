<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 3160, y: listScrollHeight }"
...
<script>
import { doLoadProjectAllSubTotalYear, exportProjectAllSubTotalYear } from '@/api/projectReport_all'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      doLoadProjectAllSubTotalYear(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-A02' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectAllSubTotalYear(Object.assign(params, this.queryParam)).then(res => {
