<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2370, y: listScrollHeight }"
...
<script>
import { doLoadContractAllSubTotal, exportContractAllSubTotal } from '@/api/projectReport_all'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      doLoadContractAllSubTotal(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-A07' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractAllSubTotal(Object.assign(params, this.queryParam)).then(res => {
