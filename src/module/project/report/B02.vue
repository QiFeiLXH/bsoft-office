<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 6810, y: listScrollHeight }">
      >
...
import { getProjectLevelworkloadAllByYear, exportProjectLevelworkloadAllByYear } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getProjectLevelworkloadAllByYear(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-B02' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelworkloadAllByYear(params).then(res => {
