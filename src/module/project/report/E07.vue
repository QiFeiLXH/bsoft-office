<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2000, y: listScrollHeight }"
...
<script>
import { getContractLevelBonusAllSubtotal, exportContractLevelBonusAllSubtotal } from '@/api/projectReport_bonus'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      getContractLevelBonusAllSubtotal(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E07' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelBonusAllSubtotal(params).then(res => {
