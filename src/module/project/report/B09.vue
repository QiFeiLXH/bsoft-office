<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 6170, y: listScrollHeight }">
      >
...
import { getContractLevelworkloadAllByMonth, exportContractLevelworkloadAllByMonth } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getWidthAndHeight () {
      ...
      this.listScrollHeight = window.innerHeight - 362
    },
    ...
    getList () {
      ...
      getContractLevelworkloadAllByMonth(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-B09' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelworkloadAllByMonth(params).then(res => {
