<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 6880, y: listScrollHeight }">
      >
...
import { getProjectLevelworkloadAllByDepByYear, exportProjectLevelworkloadAllByDepByYear } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getProjectLevelworkloadAllByDepByYear(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-B05' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelworkloadAllByDepByYear(params).then(res => {
