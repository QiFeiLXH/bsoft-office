<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2960, y: listScrollHeight }"
...
<script>
import { getProjectLevelBonusDepYear, exportProjectLevelBonusDepYear } from '@/api/projectReport_bonus'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      getProjectLevelBonusDepYear(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E05' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelBonusDepYear(params).then(res => {
