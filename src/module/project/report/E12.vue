<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 2400, y: listScrollHeight }"
...
<script>
import { getContractLevelBonusDepMonth, exportContractLevelBonusDepMonth } from '@/api/projectReport_bonus'
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        {
          key: 'projectBonusQuarter',
          width: 100,
          dataIndex: 'projectBonusQuarter',
          title: '项目奖金季度',
          align: 'center'
        },
        {
          key: 'projectBonusMonth',
          width: 140,
          dataIndex: 'projectBonusMonth',
          title: '项目奖金核算月份',
          align: 'center'
        },
        {
  ...
  methods: {
    ...
    doQuery () {
      this.isLoading = true
      getContractLevelBonusDepMonth(this.queryParam)
        .then(res => {
          if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-E12' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportContractLevelBonusDepMonth(params).then(res => {
