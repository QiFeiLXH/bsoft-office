<template>
  <a-card
    ...
    <a-table
      ...
      :scroll="{ x: 6740, y: listScrollHeight }">
      >
...
import { getProjectLevelworkloadAll, exportProjectLevelworkloadAll } from '@/api/projectReport_manpowercost'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getProjectLevelworkloadAll(params).then(res => {
        if (res.code === 200) {
    ...
    doExport () {
      ...
      params.context = this.$route.meta.title + '-B01' // 菜单名称
      params.personId = store.getters.userInfo.personId // 用户id
      params.system = 13 // 系统
      this.isDownloading = true
      exportProjectLevelworkloadAll(params).then(res => {
