...
const err = (error) => {
  ...
  // if (error.response) {
    ...
    //   })
    // }
    // if (error.response.status === 401 && !(data.body && data.body.isLogin)) {
    //   notification.error({
...
service.interceptors.request.use(config => {
  ...
  if (config.headers['Content-Type'] == ('application/json;charset=UTF-8') && cryptFlag == 1) {
    ...
    let encryptedRandom = rsaEncryptByPublicKey(random)
...
service.interceptors.response.use((response) => {
  ...
  if (response.config.url.endsWith('Download') || response.config.url.endsWith('download')) {
    ...
    if (cryptFlag == 1) {
      let decryptPassword = rsaDecryptByPublicKey(response.headers["des-password"])
      let dataStr = tripleDESDecrypt(decryptPassword, response.data);
